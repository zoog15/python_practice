...
class FixedQueue:
    ...
    def enque(self, x: Any) -> None:
        ...
        self.que[self.rear] = x  # 현재 rear에 값 넣기
        self.rear += 1  # rear가 다음 칸 가르키기
        self.no += 1  # 현재 데이터 갯수 1 증가
        if self.rear == self.capacity:  # 만약에 rear가 큐의 끝에 도달했다면 맨 처음으로 되돌리기
            self.rear = 0
    ...
    def find(self, value: Any) -> Any:
        ...
        for i in range(self.no):
            idx = (i + self.front) % self.capacity  # 이렇게 해야 front부터 1씩 증가해가며 찾을 수 있음
